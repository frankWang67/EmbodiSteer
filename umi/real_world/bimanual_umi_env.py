...
from umi.common.urdf_util import resolve_curobo_ee_transform

try:
    from diffusion_policy.real_world.multi_realsense import MultiRealsense, SingleRealsense
    from diffusion_policy.real_world.video_recorder import VideoRecorder as RealSenseVideoRecorder
    REALSENSE_IMPORT_ERROR = None
except ModuleNotFoundError as ex:
    MultiRealsense = None
    SingleRealsense = None
    RealSenseVideoRecorder = None
    REALSENSE_IMPORT_ERROR = ex
...
class BimanualUmiEnv:
    def __init__(self, 
            ...
            mirror_swap=False,
            # optional sidecar realsense recording
            record_aux_realsense=False,
            aux_realsense_serial_numbers=None,
            aux_realsense_video_dirname='realsense_videos',
            aux_realsense_resolution=(1280, 720),
            aux_realsense_capture_fps=30,
            aux_realsense_record_fps=None,
            aux_realsense_video_crf=21,
            aux_realsense_thread_count=2,
        ...
        aux_realsense_video_dir = None
        if record_aux_realsense:
            aux_realsense_video_dir = output_dir.joinpath(aux_realsense_video_dirname)
            aux_realsense_video_dir.mkdir(parents=True, exist_ok=True)
        ...
        aux_realsense = None
        if record_aux_realsense:
            if REALSENSE_IMPORT_ERROR is not None:
                raise RuntimeError(
                    'record_aux_realsense=True, but RealSense dependencies are unavailable.'
                ) from REALSENSE_IMPORT_ERROR
            if aux_realsense_serial_numbers is None:
                aux_realsense_serial_numbers = SingleRealsense.get_connected_devices_serial()
            if len(aux_realsense_serial_numbers) == 0:
                raise RuntimeError(
                    'record_aux_realsense=True, but no RealSense cameras were detected.'
                )
            if aux_realsense_record_fps is None:
                aux_realsense_record_fps = aux_realsense_capture_fps
            aux_realsense_video_recorder = RealSenseVideoRecorder.create_h264(
                fps=aux_realsense_record_fps,
                codec='h264',
                input_pix_fmt='bgr24',
                crf=aux_realsense_video_crf,
                thread_type='FRAME',
                thread_count=aux_realsense_thread_count
            )
            aux_realsense = MultiRealsense(
                serial_numbers=aux_realsense_serial_numbers,
                shm_manager=shm_manager,
                resolution=aux_realsense_resolution,
                capture_fps=aux_realsense_capture_fps,
                put_fps=aux_realsense_capture_fps,
                put_downsample=False,
                record_fps=aux_realsense_record_fps,
                enable_color=True,
                enable_depth=False,
                ...
                video_recorder=aux_realsense_video_recorder,
                verbose=False
            )
            ...
            if rc['robot_type'].startswith('ur5'):
                ...
                if 'curobo_robot_config' in rc:
                    ...
                    if controller_ee_link is None:
                        raise ValueError(
                            "Franka config with curobo_robot_config must define "
                            "controller_ee_link, e.g. 'panda_link8'."
                        )
                    tx_controller_ee_tip, ee_link, urdf_path = resolve_curobo_ee_transform(
                        robot_config_path=rc['curobo_robot_config'],
                        controller_ee_link=controller_ee_link,
                    )
                    print(
                        f"Resolved Franka TCP transform: {controller_ee_link} -> "
                        f"{ee_link} from {urdf_path}"
                    )
        ...
        self.aux_realsense_video_dir = aux_realsense_video_dir
    ...
    def is_ready(self):
        ...
        if self.aux_realsense is not None:
            ready_flag = ready_flag and self.aux_realsense.is_ready
    ...
    def start(self, wait=True):
        ...
        if self.aux_realsense is not None:
            self.aux_realsense.start(wait=False)
        for robot in self.robots:
    ...
    def stop(self, wait=True):
        ...
        if self.aux_realsense is not None:
            self.aux_realsense.stop(wait=False)
    ...
    def exec_actions(self, 
        ...
        if self.action_accumulator is not None:
            action_key = 'joint_action' if action_mode == 'joint' else 'action'
            if self.action_key is None:
                self.action_key = action_key
            elif self.action_key != action_key:
                raise RuntimeError(
                    f"Cannot mix action modes in one episode: "
                    f"existing key {self.action_key!r}, new key {action_key!r}."
                )
    ...
    def get_aux_realsense_rgb(self, camera_idx=0, output_res=None):
        if self.aux_realsense is None:
            return None
        if camera_idx < 0 or camera_idx >= self.aux_realsense.n_cameras:
            return None

        data = self.aux_realsense.get(k=1)
        img = data[camera_idx]['color'][-1]
        if output_res is not None:
            img = cv2.resize(img, output_res)
        # RealSense frames are stored as BGR; convert to RGB for consistency
        # with the existing obs images used by eval_real.py.
    ...
    def start_episode(self, start_time=None):
        ...
        if self.aux_realsense is not None:
            assert self.aux_realsense_video_dir is not None
            this_realsense_video_dir = self.aux_realsense_video_dir.joinpath(str(episode_id))
            this_realsense_video_dir.mkdir(parents=True, exist_ok=True)
            aux_video_paths = list()
            for i in range(self.aux_realsense.n_cameras):
                aux_video_paths.append(
                    str(this_realsense_video_dir.joinpath(f'{i}.mp4').absolute())
                )
            self.aux_realsense.restart_put(start_time=start_time)
            self.aux_realsense.start_recording(
                video_path=aux_video_paths,
    ...
    def end_episode(self):
        ...
        if self.aux_realsense is not None:
            self.aux_realsense.stop_recording()
        ...
        if self.obs_accumulator is not None:
            ...
            if n_steps > 0:
                ...
                episode = {
                    ...
                    action_key: actions[:n_steps],
                }
                for other_action_key in ('action', 'joint_action'):
                    if other_action_key == action_key:
                        continue
                    if other_action_key not in self.replay_buffer.data:
                        continue
                    other_action_arr = self.replay_buffer.data[other_action_key]
                    episode[other_action_key] = np.full(
                        (n_steps,) + other_action_arr.shape[1:],
                        np.nan,
                        dtype=other_action_arr.dtype
                    )
    ...
    def drop_episode(self):
        ...
        if self.aux_realsense_video_dir is not None:
            this_realsense_video_dir = self.aux_realsense_video_dir.joinpath(str(episode_id))
            if this_realsense_video_dir.exists():
                shutil.rmtree(str(this_realsense_video_dir))
