...
@click.option('-c', '--calibration_dir', required=True, type=click.Path(exists=True, file_okay=False),
              help='Directory containing GoPro intrinsics and ArUco configuration.')
@click.option('--openicc_image', required=True,
              help='OpenICC Docker image used to extract GoPro IMU metadata.')
@click.option('--orb_slam_image', required=True,
              help='ORB-SLAM3 Docker image used for mapping and trajectory recovery.')
def main(session_dir, calibration_dir, openicc_image, orb_slam_image):
